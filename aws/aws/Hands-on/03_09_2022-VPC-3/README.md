...
- Create a bucket of `hrn-vpc-endpoint` with following properties, 
...
Role Name : hrnS3FullAccessforEndpoint
Role description: hrn S3 Full Access for Endpoint
click create button
```
...
- list the bucket in S3 and content of S3 bucket named "aws s3 ls "hrn-vpc-endpoint" via following command

```
aws s3 ls
aws s3 ls hrn-vpc-endpoint
```
...
"hrn-vpc-endpoint" via following command
```bash
aws s3 ls
aws s3 ls hrn-vpc-endpoint
```
...
```bash
aws s3 cp s3://hrn-vpc-endpoint/Guile.png .
aws s3 cp s3://hrn-vpc-endpoint/Honda.png .
```
